...
#include "caffe/layers/npair_data_same_second_label_layer.hpp"
...
vector<pair<string, int> > NPairDataSSLLayer<Dtype>::RandLeafImages(int leaf_label, int rand_image_num) {
...
template <typename Dtype>
NPairDataSSLLayer<Dtype>::~NPairDataSSLLayer<Dtype>() {
  // this->JoinPrefetchThread();
  this->StopInternalThread();
}

template <typename Dtype>
void NPairDataSSLLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  int leaf;
  int sec_label;
  set<int> sec_label_set;

  while (std::getline(infile, line)) {
    std::istringstream iss(line);
    leaf = INVALID_VALUE;
    iss >> filename >> leaf >> sec_label;
    if (leaf == INVALID_VALUE || leaf == -1)
      continue;

    sec_label_set.insert(sec_label);
    if (sec_label_leaf_map_.find(sec_label) == sec_label_leaf_map_.end()) {
      set<int> leafs;
      sec_label_leaf_map_[sec_label] = leafs;
    }
    sec_label_leaf_map_[sec_label].insert(leaf);
  ...
  sec_labels_.assign(sec_label_set.begin(), sec_label_set.end());
...
template<typename Dtype>
void NPairDataSSLLayer<Dtype>::ShuffleSecLabels() {
  caffe::rng_t* prefetch_rng =
    static_cast<caffe::rng_t*>(prefetch_rng_->generator());
  shuffle(sec_labels_.begin(), sec_labels_.end(), prefetch_rng);
}

template<typename Dtype>
vector<int> NPairDataSSLLayer<Dtype>::ShuffleLeafsWithSecLabels(int sec_label_num) {
  vector<int> certain_leafs;
  caffe::rng_t* prefetch_rng =
    static_cast<caffe::rng_t*>(prefetch_rng_->generator());
  certain_leafs.assign(sec_label_leaf_map_[sec_label_num].begin(), sec_label_leaf_map_[sec_label_num].end());
  shuffle(certain_leafs.begin(), certain_leafs.end(), prefetch_rng);
  return certain_leafs;
}

template<typename Dtype>
void NPairDataSSLLayer<Dtype>::ShuffleImages(int leaf_label) {
  caffe::rng_t* prefetch_rng =
    static_cast<caffe::rng_t*>(prefetch_rng_->generator());
...
template <typename Dtype>
void NPairDataSSLLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  int sec_label_num = sec_labels_[0];
  vector<int> leaf_labels = ShuffleLeafsWithSecLabels(sec_label_num);

  int item_id = 0;
  while (true) {
    bool is_finish = false;

    int leaf_label = leaf_labels[leaf_cursor_];
    leaf_cursor_ ++;
...
INSTANTIATE_CLASS(NPairDataSSLLayer);
REGISTER_LAYER_CLASS(NPairDataSSL);
