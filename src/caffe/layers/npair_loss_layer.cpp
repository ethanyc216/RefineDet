...
#include "caffe/layers/npair_loss_layer.hpp"
#include <time.h>

namespace caffe {

template <typename Dtype>
void NPairLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  gamma_ = 0.001;
}

template <typename Dtype>
void NPairLossLayer<Dtype>::Reshape(
...
template <typename Dtype>
void NPairLossLayer<Dtype>::CalSquEucDist(const int qry_num, const int ref_num, const int dim,
...
template <typename Dtype>
Dtype NPairLossLayer<Dtype>::CalLossAndWeightMatrix(const int batch_size, const Dtype* labels,
    map<Dtype, vector<int> > reference_labels_idx, Dtype* _W) {
  Dtype loss = 0;

  for (int qry_idx=0; qry_idx<batch_size; qry_idx++) {
    Dtype qry_label = labels[qry_idx];
...
template <typename Dtype>
Dtype NPairLossLayer<Dtype>::CalLossAndWeightRow(int qry_idx, int pos_idx, const int batch_size,
    Dtype* _Wq) {
...
template <typename Dtype>
void NPairLossLayer<Dtype>::Backward_cpu(
