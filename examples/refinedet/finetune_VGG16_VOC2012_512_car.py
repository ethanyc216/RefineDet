...
# snapshot prefix.
snapshot_prefix = "{}/".format(snapshot_dir)
# job script path.
job_file = "run.sh"

# Stores the test image names and sizes. Created by data/VOC0712Plus/create_list.sh
name_size_file = "val_name_size.list"
# The pretrained model. We use the RefineDet model trained on coco dataset.
pretrain_model = "VOC0712Plus_refinedet_vgg16_512x512_ft_final.caffemodel"
# Stores LabelMapItem.
label_map_file = "labelmap_car.prototxt"
...
batch_size = 16 * len(gpus.strip().split(','))
accum_batch_size = 16 * len(gpus.strip().split(','))
...
AddExtraLayers(net, use_batchnorm, arm_source_layers, normalizations, lr_mult=1)
arm_source_layers.reverse()
normalizations.reverse()
...
with open(job_file, 'w') as f:
  ...
  f.write('#!/bin/bash\n')
  f.write('HERE=`pwd`\n')
  f.write('source $HERE/conf.sh\n\n')
  f.write('CAFFE_HOME=$HERE/distribute\n')
  f.write('export LD_LIBRARY_PATH=${HERE}/lib:${CAFFE_HOME}/lib:$LD_LIBRARY_PATH\n')
  f.write('export LD_LIBRARY_PATH=/home/work/cuda-7.5/lib64:$LD_LIBRARY_PATH\n')
  f.write('export LD_LIBRARY_PATH=/home/work/cudnn/cudnn_v4/cuda/lib64:$LD_LIBRARY_PATH\n\n')
  f.write('export hdfs_dataset=car_det_v2/train_lmdb\n')
  f.write('sh ./hdfs.sh download ${hdfs_dataset} ./\n')
  f.write('export hdfs_dataset=car_det_v2/test_lmdb\n')
  f.write('sh ./hdfs.sh download ${hdfs_dataset} ./\n\n')
  f.write('./build/tools/caffe train \\\n')
  f.write('--solver="{}" \\\n'.format(solver_file))
  f.write(train_src_param)
  if solver_param['solver_mode'] == P.Solver.GPU:
    f.write('--gpu {} 2>&1 | tee log/train.log\n'.format(gpus))
  else:
